...
/**
 * Completed runs: delivered or customer refused (DELIVERY_RETURNED) via GET /api/v1/orders?delivery_list_scope=history.
 */
export default function DeliveryHistoryScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getAssignedOrders({ limit: 50, offset: 0, delivery_list_scope: 'history' });
      setItems(res.items ?? []);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={[profileStyles.safe, styles.pad]} edges={['left', 'right']}>
      <FlatList
        ...
        ListHeaderComponent={
          <>
            <Text style={profileStyles.h1}>History</Text>
            <Text style={profileStyles.sub}>Delivered orders and returns when the customer did not accept.</Text>
        ...
        ListEmptyComponent={
          !error ? (
            <Text style={profileStyles.msg}>No completed deliveries yet. Pull to refresh.</Text>
          ) : null
        }
        renderItem={({ item }) => (
          <Pressable
            ...
            <Text style={styles.tapHint}>Open for details →</Text>
          </Pressable>
        )}
      />
    </SafeAreaView>
